...
import { TDomainPrivateMethod, TParameterDependencies, TReturnType, TOkErrorReturnType } from '../../../types.js';
...
export const privateMethodDeclarationVisitor = (
  ...
  const parameterDependencies: TParameterDependencies = thisVisitor.visit(ctx.formalParameterList());
  const returnType: TReturnType | TOkErrorReturnType = thisVisitor.visit(ctx.returnPrivateMethodType());
